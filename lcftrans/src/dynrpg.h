...
using dyn_arg_list = const lcf::Span<std::string>;
...
namespace DynRpg {
	namespace detail {
		template <typename T>
		inline bool parse_arg(lcf::StringView, dyn_arg_list, const int, T&, bool&) {
		...
		template <>
		inline bool parse_arg(lcf::StringView func_name, dyn_arg_list args, const int i, float& value, bool& parse_okay) {
		...
		template <>
		inline bool parse_arg(lcf::StringView func_name, dyn_arg_list args, const int i, int& value, bool& parse_okay) {
			if (!parse_okay) return false;
			value = 0;
			if (args[i].empty()) {
				parse_okay = true;
		...
		template <>
		inline bool parse_arg(lcf::StringView, dyn_arg_list args, const int i, std::string& value, bool& parse_okay) {
			if (!parse_okay) return false;
			value = args[i];
			parse_okay = true;
			return parse_okay;
		...
		inline void parse_args(lcf::StringView func_name, dyn_arg_list in, Tuple& value, bool& parse_okay, std::index_sequence<I...>) {
	...
	std::string ParseVarArg(lcf::StringView func_name, dyn_arg_list args, int index, bool& parse_okay);
	std::string ParseCommand(const std::string& command, std::vector<std::string>& params);
	std::vector<std::string> Invoke(std::string& command);
	bool Invoke(const std::string& func, dyn_arg_list args);
	void Update();
	void Reset();

	template <typename... Targs>
	std::tuple<Targs...> ParseArgs(lcf::StringView func_name, dyn_arg_list args, bool* parse_okay = nullptr) {
		std::tuple<Targs...> t;
